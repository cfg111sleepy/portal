...
@Component({
  selector: '[app-psg-svg-widjet10-hour]',
  templateUrl: './psg-svg-widjet10-hour.component.svg',
  styleUrls: ['./psg-svg-widjet10-hour.component.scss']
})
export class PsgSvgWidjet10HourComponent implements OnInit, OnChanges {
  ...
  ngOnChanges(changes: SimpleChanges) {
    if (changes["data"] && this.data || changes["displayTotal"]) {
      ...
      if(this.display10hh) {
        values = this.data.get(`${this.aktKey_10}`);
        activeValues = this.data.get(`${this.texAktKey_10}`);
        longValues = this.data.get(`${this.longKey_10}`);
  
      } else {
        values = this.data.get(`${this.aktKey}`);
        activeValues = this.data.get(`${this.texAktKey}`);
        longValues = this.data.get(`${this.longKey}`);
      }
  ...
  onClickTrend(event: Event) {
    event.stopPropagation();
    this.clickTrend.emit(this.aktKey);
  }
